...
func encrypt(bytes: PackedByteArray, key: PackedByteArray) -> PackedByteArray:
	System.logger.error("EncryptionStrategy.encrypt() must be implemented by subclasses.")
...
func decrypt(bytes: PackedByteArray, key: PackedByteArray) -> PackedByteArray:
	System.logger.error("EncryptionStrategy.decrypt() must be implemented by subclasses.")
	return PackedByteArray()
