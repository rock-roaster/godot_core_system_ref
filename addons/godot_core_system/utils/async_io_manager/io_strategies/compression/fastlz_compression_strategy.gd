...
# 压缩模式常量
const COMPRESSION_MODE: int = FileAccess.COMPRESSION_FASTLZ

# 估算解压缩缓冲区大小的乘数 (如果经常出错，需要调整)
const DECOMPRESSION_BUFFER_MULTIPLIER: int = 10
# 解压缩缓冲区最小大小 (防止估算为0)
const MIN_DECOMPRESSION_BUFFER_SIZE: int = 1024
...
func decompress(bytes: PackedByteArray) -> PackedByteArray:
	...
	var estimated_size: int = bytes.size() * DECOMPRESSION_BUFFER_MULTIPLIER
	# 确保缓冲区大小至少为最小值
	...
	if decompressed_bytes.is_empty() and not bytes.is_empty():
		push_warning("FastLZ Decompression resulted in empty bytes. Buffer size (%d) might be insufficient or data corrupted." % estimated_size)
