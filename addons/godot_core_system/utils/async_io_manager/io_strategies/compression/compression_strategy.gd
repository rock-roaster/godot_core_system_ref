extends RefCounted

## Abstract method to compress byte data.
func compress(bytes: PackedByteArray) -> PackedByteArray:
	System.logger.error("CompressionStrategy.compress() must be implemented by subclasses.")
	return PackedByteArray()

## Abstract method to decompress byte data.
func decompress(bytes: PackedByteArray) -> PackedByteArray:
	System.logger.error("CompressionStrategy.decompress() must be implemented by subclasses.")
	return PackedByteArray()
