...
func activate(initial_context : Dictionary = {}) -> void:
	if _is_active:
		System.logger.warning("Trigger is already active")
		return
	_is_active = true
	...
	match trigger_type:
		...
		TRIGGER_TYPE.ON_EVENT:
			if trigger_event.is_empty():
				System.logger.warning("Trigger event is empty")
			else:
...
func deactivate() -> void:
	if not _is_active:
		System.logger.warning("Trigger is not active")
		return
	_is_active = false
...
func _try_trigger(context: Dictionary) -> void:
	...
	if max_triggers > 0 and trigger_count >= max_triggers:
		System.logger.debug("Trigger count reached limit")
		return

	# 概率不满足
	if randf() > trigger_chance:
		System.logger.debug("Trigger chance not satisfied")
		return

	## 条件不满足
	if not should_trigger(context):
		System.logger.debug("Trigger conditions not satisfied")
		return
