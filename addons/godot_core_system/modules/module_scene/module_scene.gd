...
var _current_scene: Node:
	...
	set(value): _current_tree.set_current_scene(value)
...
func clear_scene_stack() -> void:
	for stack in _scene_stack.values():
		stack.get("scene_node").queue_free()
	_scene_stack.clear()
...
func get_scene_node(scene_path: String, scene_data: Dictionary = {}) -> Node:
	...
	if is_instance_valid(new_scene):
		if new_scene.has_method("_on_scene_restore"):
			new_scene.call("_on_scene_restore", scene_data)
	...
	# 检查场景构造函数
	if new_scene != null:
		if new_scene.has_method("_on_scene_init"):
			new_scene.call("_on_scene_init", scene_data)
...
func push_scene_to_stack(scene: Node) -> void:
	...
	var scene_data: Dictionary = get_scene_save(scene)
...
func change_scene(
	...
	await _do_scene_switch(new_scene, push_to_stack)

	if new_scene.has_method("_on_scene_ready"):
		new_scene.call("_on_scene_ready")
	...
	scene_switching_finished.emit(scene_path)
	if new_scene.has_method("_on_scene_switch"):
		new_scene.call("_on_scene_switch")
